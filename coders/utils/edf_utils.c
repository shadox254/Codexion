...
/*   Updated: 2026/03/20 09:43:29 by rruiz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	wait_for_dongles(t_coder *coder, t_dongle *dongle1,
				t_dongle *dongle2);
		...
		t_dongle *dongle2)
{
	...
	while (is_simu(coder->data))
	{
		...
		{
			now = get_time();
			if (now >= dongle1->last_release
				+ coder->data->rules.dongle_cooldown && now
				>= dongle2->last_release + coder->data->rules.dongle_cooldown)
				break ;
